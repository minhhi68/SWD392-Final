...
public class BanAnDAO {
    ...
    public List<BanAnDTO> LayTatCaBanAn(){
        ...
        while (!cursor.isAfterLast()){
            ...
            banAnDTO.setIdMaNv(String.valueOf(cursor.getInt(cursor.getColumnIndex(CreateDatabase.TBL_BAN_ID_MANV))));
            banAnDTO.setDate(cursor.getString(cursor.getColumnIndex(CreateDatabase.TBL_BAN_NGAYDAT)));
    ...
    //cập nhật idMaNV
    public boolean CapNhatMaNV(int maban, int manv, String date){
        ContentValues contentValues = new ContentValues();
        contentValues.put(CreateDatabase.TBL_BAN_ID_MANV,manv);
        contentValues.put(CreateDatabase.TBL_BAN_NGAYDAT,date);
        long ktra = database.update(CreateDatabase.TBL_BAN,contentValues,CreateDatabase.TBL_BAN_MABAN+ " = '"+maban+"' ",null);
        if(ktra != 0){
            return true;
        }else {
    ...
    //get all by id manv
    public List<BanAnDTO> LayTatCaBanAnTheoMaNV(int manv){
        List<BanAnDTO> banAnDTOList = new ArrayList<BanAnDTO>();
        String query = "SELECT * FROM " +CreateDatabase.TBL_BAN + " WHERE " +CreateDatabase.TBL_BAN_ID_MANV+ " = '" +manv+ "' ";
        Cursor cursor = database.rawQuery(query,null);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()){
            BanAnDTO banAnDTO = new BanAnDTO();
